...
(custom-set-variables
 ...
 '(package-selected-packages
   (quote
    (tss tide evil-magit magit sr-speedbar smartparens slime skewer-mode rvm rsense relative-line-numbers rainbow-mode rainbow-delimiters paredit org-ac org noflet mwe-log-commands load-theme-buffer-local linum-relative key-chord jedi hlinum evil ess-R-object-popup ein ecb e2wm dired-rainbow company color-theme clojure-cheatsheet ac-inf-ruby ac-cider ac-anaconda)))
...
;; 2015-03-09. global hot key for toggling smartparens on/off.
...
;; add comment key binding 2015-12-25
(global-set-key (kbd "C-c v c") 'comment-region)
...
;; run customize-face to see
;; however, I just add new colors directly here
(custom-set-faces
 ...
 '(vt-face-orange-\#ffa500 ((t (:background "#ffa500" :foreground "#ffa500"))) t)
...
;; 2015-12-25
;; set the modes you do not want to be evil-mode
;; note: these function are only available once you are in evil mode
;;  Since I don't want to globally enable evil just yet, I'll just manually
;;  invoke emacs mode on a buffer by buffer basis with C-z (evil-emacs-state)
;;(evil-set-initial-state 'lisp-interaction-mode 'emacs)
;;(add-to-list 'evil-emacs-state-modes 'lisp-interaction-mode)


(evil-mode)
;; evil (vi) convenience method
;;  ((or (evil-insert-state-p) (evil-normal-state-p) (evil-replace-state-p) (evil-visual-state-p))
(defun vt-add-semicolon-at-end-of-line()
  (interactive)
  (let ((i 0)(p (point))) 
    (while (and (< i 300) (not (equal (char-after p) ?\n)))
      ;;(message "p=%d, c=%d" p (char-after p))
      (setq p (1+ p))
      (setq i (1+ i))
     )
    ;;(message "found newline at %d" p)
    ;; eliminate up to one trailing space
    (when (equal (char-before p) ?\ )
       (progn
         ;; (message "found space at %d" p)
         (setq p (1- p))
         (goto-char p)
         (delete-char 1)))
    (goto-char p)
    (insert ?\;)
    ;; and hit the 'esc key if running in evil mode-line
    (when (fboundp 'evil-force-normal-state)
      (evil-force-normal-state))
    )
)

(global-set-key (kbd "C-c v ;") 'vt-add-semicolon-at-end-of-line)

;; evil convenience: get out of a bracket valley e.g (<cursor_here>)
(defun vt-escape-bracket-valley()
  (interactive)
  (let ((i 0)(p (point))) 
    (while (and (< i 100)
                (not (equal (char-after p) ?\) ))
                (not (equal (char-after p) ?\] ))
                (not (equal (char-after p) ?\} ))
           )
      (setq p (1+ p))
...
(global-set-key (kbd "C-c v n") 'vt-escape-bracket-valley)
;;(define-key evil-insert-state-map (kbd "jj") 'evil-normal-state)

(key-chord-mode 1)
;; just use the defualt of 0.2
;;(setq key-chord-two-keys-delay 0.5)

(key-chord-define evil-insert-state-map "jj" 'evil-normal-state)

;;add an insert new line w/o insert mode-line
;; equivalent to: nmap oo o<Esc>k 
(key-chord-define (current-global-map) "oo"
  (lambda () 
    (interactive)
    (evil-open-below 1)
    (evil-normal-state)
...
;; allow c-n and c-p to move up and and in vi edit mode
;; note: c-f and c-b are alread defined. 
;; this effectively replaces the need to  vt-escape-bracket-valley
(define-key evil-insert-state-map (kbd "C-p") 'evil-previous-line)
(define-key evil-insert-state-map (kbd "C-n") 'evil-next-line)

;; this one works fine on windows, but is a little slow on linux vmware
...
;; since c-u is bound to the unit key on emacs, bind
;; scroll half screen up to c-D
(global-set-key (kbd "C-S-d") 'evil-scroll-up )

;;2016-01-19 enable evil-magit
(require 'evil-magit)
(global-set-key (kbd "C-c g") 'magit-status )

;; get some better evil keybindins in ibuffer
...
(add-hook 'typescript-mode-hook
          (lambda ()
            ...
            (company-mode-on)
            ;;vt add
            (define-key typescript-mode-map (kbd "M-N") 'scroll-up-command)
            (define-key typescript-mode-map (kbd "M-P") 'scroll-down-command)
            ;;vt end
            ))

;; 2016-04-02
;; add paging up down in a automcomplete pop ups
;; we just bind to the same keys as <pgUp> and <pgDn>
;;(add-hook 'typescript-mode-hook
;;          (lambda ()
...
;; 2016-04-07
;; attempt to fix angular-2 cli duplicate entry problem
;; see http://ember-cli.com/user-guide/#commonissues
